  ...
  return (
    <div className="flex flex-col items-center w-full max-w-2xl mx-auto">
      ...
      <div className="flex flex-col items-center gap-3 mt-4 w-full">
         {/* Color Palette */}
         <div className="flex gap-3 px-4 py-2 bg-white/80 backdrop-blur rounded-full shadow-sm border border-gray-100 overflow-x-auto max-w-full">
            {CHRISTMAS_COLORS.map(color => (
                <button
                    key={color}
                    onClick={() => handleColorSelect(color)}
                    className={`flex-shrink-0 w-8 h-8 rounded-full border-2 transition-all ${activeColor === color && tool === 'pen' ? 'border-gray-800 scale-125 shadow-md' : 'border-transparent hover:scale-110'}`}
         ...
         {/* Tools */}
         <div className="flex gap-2 p-2 bg-white/80 backdrop-blur rounded-full shadow-lg border border-gray-100">
            <button 
              onClick={() => setTool('pen')}
